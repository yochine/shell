...
public class SM2Util extends GMBaseUtil {
     ...
     * @throws InvalidCipherTextException
     */
    public static byte[] encrypt(Mode mode, String sm2PublicKey, byte[] srcData) throws InvalidCipherTextException, NoSuchAlgorithmException, NoSuchProviderException, InvalidKeySpecException {
        //公钥复原 将X509标准的公钥字节流转为公钥对象
        BCECPublicKey pubKey = BCECUtil.encodEcPublicKey(HexUtil.decodeHex(sm2PublicKey));
        return encrypt(mode,pubKey,srcData);
